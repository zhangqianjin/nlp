...
"""
@Author:张前进
@description:使用textrank得到特征词，构建词典，使用one-hot编码技术(或者使用词频)构建特征向量,使用特征向量训练模型得到相应的模型参数
...
if __name__ == "__main__":
    ...
    bad_segment_list, bad_word_set, bad_content_list = readdata(bad_info_filename, stopword_list)
    good_segment_list, good_word_set, good_content_list = readdata(good_info_filename, stopword_list)
    """
    获得词典
    """
    bad_word_bag_weight = jieba.analyse.textrank(",".join(bad_content_list), topK=50, withWeight=True)
    good_word_bag_weight = jieba.analyse.textrank(",".join(good_content_list), topK=50, withWeight=True)
    Word_Dict= []
    for word, weight in good_word_bag_weight:
        Word_Dict.append(word)
    for word, weight in bad_word_bag_weight:
        Word_Dict.append(word)
